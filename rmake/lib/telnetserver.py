...
class TelnetServerForCommand(TelnetServer):
    def __init__(self, server_address=None, command=['/bin/sh'],
                 workDir='/',
                 requestHandlerClass=TelnetRequestHandler):
        class CommandRequestHandler(requestHandlerClass):
            pass

        if not isinstance(command, list):
            command = [command]

        CommandRequestHandler.command = command[0]
        CommandRequestHandler.args = command
        CommandRequestHandler.workDir = workDir
        TelnetServer.__init__(self, server_address, CommandRequestHandler)
